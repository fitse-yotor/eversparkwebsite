...
export async function getHeroImages(): Promise<HeroImage[]> {
  ...
  const { data, error } = await supabase
    .from("hero_images")
    .select("id, image_url, title, subtitle, description, sort_order, is_active")
    .eq("is_active", true)
  ...
  console.log("Successfully fetched hero images.")
  return (
    data?.map((item) => ({
      id: item.id,
...
export async function addHeroImage(heroImage: Omit<HeroImage, "id">) {
  const supabase = await createSupabaseServerClient()
  const { error } = await supabase.from("hero_images").insert({
    ...
    sort_order: heroImage.sortOrder,
    is_active: heroImage.isActive,
  })

  if (error) {
    console.error("Error adding hero image:", error.message)
    return { success: false, message: `Failed to add hero image: ${error.message}` }
  }

  revalidatePath("/admin/content")
  revalidatePath("/")
  return { success: true, message: "Hero image added successfully." }
}

export async function updateHeroImage(id: string, heroImage: Omit<HeroImage, "id">) {
  const supabase = await createSupabaseServerClient()
  const { error } = await supabase
    .from("hero_images")
    .update({
  ...
  if (error) {
    ...
    return { success: false, message: `Failed to update hero image: ${error.message}` }
  }
...
export async function deleteHeroImage(id: string) {
  ...
  if (error) {
    ...
    return { success: false, message: `Failed to delete hero image: ${error.message}` }
  }
